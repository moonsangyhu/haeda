...
@pytest.mark.asyncio
async def test_update_profile_day_cutoff_hour_valid(client: AsyncClient, user: User):
    """day_cutoff_hour=2 → 200 + 저장, 응답에 day_cutoff_hour 포함"""
    resp = await client.put(
        "/api/v1/auth/profile",
        data={"day_cutoff_hour": 2},
        headers={"Authorization": f"Bearer {user.id}"},
    )
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert data["day_cutoff_hour"] == 2


@pytest.mark.asyncio
async def test_update_profile_day_cutoff_hour_invalid(client: AsyncClient, user: User):
    """day_cutoff_hour=3 → 422 INVALID_DAY_CUTOFF_HOUR"""
    resp = await client.put(
        "/api/v1/auth/profile",
        data={"day_cutoff_hour": 3},
        headers={"Authorization": f"Bearer {user.id}"},
    )
...
@pytest.mark.asyncio
async def test_kakao_login_response_includes_day_cutoff_hour(
    client: AsyncClient, db_session: AsyncSession
):
    """카카오 로그인 응답 user 객체에 day_cutoff_hour 포함 (신규 유저 default 0)"""
    mock_kakao = {
        "id": 9999042,
        "kakao_account": {
            "profile": {
    ...
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert "day_cutoff_hour" in data["user"]
    assert data["user"]["day_cutoff_hour"] == 0
